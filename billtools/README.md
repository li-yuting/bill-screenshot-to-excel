...
| 单笔交易详情页 | 每张图一笔交易。已支持两种版式，按特征标签自动识别：「收支详情」页、建行「明细详情」页 | `明细` + `汇总`（按年/按月/按金额档/按摘要/按交易场所 + 自动校验） |
...
`detail.py` 不写死一套字段表，而是声明「认哪些标签、标签映射到哪个字段」：

```python
Profile(
    key="ccb-detail",
    title="建行「明细详情」",
    labels=("交易账户", "交易时间", "支付机构", "商户名称", "商品信息",
            "交易方式", "对方账户", "记账日", "账户余额", "交易流水号"),
    fields={"交易账户": "account", "交易时间": "time",
            "支付机构": "place_channel", "商户名称": "place_merchant",
            "账户余额": "balance", "记账日": "booking"},
    balance_label="账户余额",
    booking_label="记账日",
)
```

- **版式识别**：按图上命中的特征标签数挑档案，取最多的那个（实测老版式命中 7、建行命中 10，判据很干净）
- 没映射到核心字段的标签进 `extra`，导出时**动态加列**（流水号 / 对方账户 / 商品信息 / 交易方式）
- 「支付机构 + 商户名称」拼成 `交易场所`，使「按交易场所」汇总对两种版式都成立
- 建行版式没有「交易金额」字段，用**「记账日」是否等于「交易时间」的日期**补上交叉校验
- 定位自校准：字段区上沿 = 最靠上的标签块 y；值区下沿 = 标签 y + 0.8 × 相邻标签间距中位数

**加一种新版式** = 往 `PROFILES` 里加一条，不用改取值逻辑。
...
| `detail.py` | 工具二：单笔交易详情页 → 一笔交易（版式档案，仅标准库） |
| `excel.py` | 共用单元格样式 + `write_statement` / `write_detail` 两个导出函数 |
| `build_exe.py` / `smoke_test.py` / `run.bat` / `app.ico` | 打包脚本 / 打包后冒烟 / 双击启动 / 图标 |

`detail.py` 和 `statement.py`、`ocr.py` 都能单独跑纯函数自检，不需要图片：

```bash
python detail.py         # 两种版式的取值、拼接、版式识别、去重、交叉校验
